...
vocabulary_size = 100 # 最常用的单词总数设置在50000
embedding_size = 128  # 嵌入向量的维度
batch_size = 128      # 每次输入的inputs是128个单词,batch_size必须是num_skips的整数倍
num_skips = 4         # 用一个单词生成2个label,num_skips必须＜或=2×skip_window
skip_window = 2       # 只考虑左右各1个单词
...
def maybe_download(filename, expected_bytes):
  ...
  if not os.path.exists(filename): #ljg filename为当前目录的文件，如果是别的位置，需要指定路径
...
def read_data():
  ...
  with open("E:\\fcouttxt1.txt") as f:
